...
import { FAVORITES_PAGE_LIMIT } from '@/lib/server-config';
...
export async function POST(request) {
    try {
        ...
        await updateUserArray(userId, 'favorites', mediaId);

        return NextResponse.json({ success: true, message: "Added to favorites successfully" });
    } catch (error) {
...
export async function GET(request) {
    try {
        ...
        const limit = parseInt(searchParams.get('limit')) || FAVORITES_PAGE_LIMIT;

        const result = await getUserList(userId, 'favorites', page, limit);
